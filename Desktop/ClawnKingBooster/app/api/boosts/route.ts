...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user) {
      ...
      const { data: newUser, error: userError } = await supabase
        ...
        .insert({
          id: userId,
          fid: userFid || null,
          username: userUsername || 'anon',
          pfp_url: userPfpUrl || null,
          addresses: userFid ? [userId] : [],
          verified: !!userFid
        })
        .select('id')
        .single()
      ...
      if (userError) {
        ...
        // Try to continue anyway - might be duplicate
      } else {
        user = newUser
      }
    }
    ...
    if (!token) {
      // Create token record with provided metadata
      ...
      if (tokenName || tokenSymbol || tokenImageUrl) {
        await supabase
          .from('tokens')
          .update({
            ...(tokenName && { name: tokenName }),
            ...(tokenSymbol && { symbol: tokenSymbol }),
            ...(tokenImageUrl && { image_url: tokenImageUrl })
          })
          .eq('id', token.id)
      }
    }
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to create boost: ' + (error as Error).message },
      { status: 500 }
    )
  }
}
